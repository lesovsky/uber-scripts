...
CREATE TABLE IF NOT EXISTS software (id SERIAL PRIMARY KEY, hostname text REFERENCES servers(hostname) ON DELETE CASCADE, os text, kernel text, pg_version text, pgb_version text, databases text);
